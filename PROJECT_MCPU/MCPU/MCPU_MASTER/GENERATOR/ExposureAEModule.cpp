...
Exposures::exposure_completed_errors Exposures::man_ae_exposure_procedure(bool demo) {
    ...
    // ______________ HIGH ENERGY EXPOSURE PULSE _______________________________________________________________
    ...
    if (Exposures::getExposurePulse(1)->filter != PCB303::filter_index::FILTER_CU) {
        // Wait for the previous command termination then change the filter in the requested (non Cu)
        PCB303::waitFilterCompleted();
        PCB303::selectFilter(Exposures::getExposurePulse(1)->filter);
    }


    error = (exposure_completed_errors)generator2DPulsePreparation(ExpName, Exposures::getExposurePulse(1)->kV, Exposures::getExposurePulse(1)->mAs, large_focus, detector_synch, grid_synch, exposure_time);
    if (error != Exposures::exposure_completed_errors::XRAY_NO_ERRORS) return error;
    ...
    if (!demo) {
        ...
        // The index is the number associated to the Databank in the procedure definition. It is not the Databank index value itself!!
        if (large_focus) setExposedData(1, (unsigned char)0, PCB303::filter_index::FILTER_CU, 1);
        else setExposedData(1, (unsigned char)0, PCB303::filter_index::FILTER_CU, 0);
