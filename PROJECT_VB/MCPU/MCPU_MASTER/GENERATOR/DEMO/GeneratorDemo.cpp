...
bool Generator::generatorDemoIdle(void) {
    ...
    while (true) {
        ...
        if (xray_processing) {
            ...
            // Debug Prints
            if (exposure_err_code == ExposureModule::exposure_completed_errors::XRAY_NO_ERRORS)  Debug::WriteLine("GENERATOR EXPOSURE: Successfully completed \n");
            else  if (ExposureModule::getExposedPulse(0)->getmAs() == 0)    Debug::WriteLine("GENERATOR EXPOSURE ERROR (NO DOSE): Error code = " + exposure_err_code.ToString() + "\n");
            else  Debug::WriteLine("GENERATOR EXPOSURE ERROR (PARTIAL DOSE): Error code = " + exposure_err_code.ToString() + "\n");
...
ExposureModule::exposure_completed_errors Generator::aec_2d_exposure_procedure_demo(void) {
    ...
    Debug::WriteLine("DEMO EXPOSURE 2D AEC STARTED\n");
    ...
    Debug::WriteLine("DEMO EXPOSURE 2D AEC: PRE-PULSE\n");
    std::this_thread::sleep_for(std::chrono::milliseconds(500));
    PCB301::set_activation_buzzer(false);
    ...
    PCB301::set_activation_buzzer(true);
    // Generation of the Aec  pulse
    Debug::WriteLine("DEMO EXPOSURE 2D AEC: MAIN-PULSE\n");
    std::this_thread::sleep_for(std::chrono::milliseconds(1000));
    PCB301::set_activation_buzzer(false);
    ...
    // Exposure completed
    Debug::WriteLine("DEMO EXPOSURE 2D AEC: COMPLETED\n");
    return ExposureModule::exposure_completed_errors::XRAY_NO_ERRORS;
}
