...
/**
	\defgroup GeneratorDescription SHFD Generator Device Description
    ...
    # Power Down Sequence Management
   
    The Generator controller shall be early informed about an incoming shutdown event.
   
    In the case the Anode rotor should run in high speed and should not stopped,
    a free run can cause mechanical resonances effects that may damage the Tube bearings.
   
    The Application should call the \ref GeneratorShutdown() method to activate the
    Generator shutdown procedure.
   
    The Generator, receiving the shutdown command, will take about five seconds,
    than switch its internal status to Shutdown status.
   
    The GeneratorShutdown() procedure shall wait for the Shutdown status before to proceed,
    for a maximum waiting time of 5 seconds.
