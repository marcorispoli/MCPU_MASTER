...
// KeepAlive Settings Structure
typedef struct
{
	UINT32 OnOff;             // enable/disable KeepAlive functionality
	UINT32 KeepAliveTime;     // idle time in seconds
	UINT32 KeepAliveInterval; // interval between KeepAlive probes in seconds
	UINT32 KeepAliveCnt;	  // number of KeepAlive probes (Linux only)
} VSCAN_KEEP_ALIVE;
...
#define VSCAN_IOCTL_SET_FILTER			12
#define VSCAN_IOCTL_SET_KEEPALIVE		13
