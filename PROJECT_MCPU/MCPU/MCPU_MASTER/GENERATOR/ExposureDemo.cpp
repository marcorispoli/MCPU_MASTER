...
void ExposureModule::exposureDemoManagementLoop(void) {
    ...
    setCompletedError(exposure_err_code);
    ...
    if (exposure_err_code == exposure_completed_errors::XRAY_NO_ERRORS)  setCompletedCode(exposure_completed_options::XRAY_SUCCESS);// xray_complete_event(exposure_completed_options::XRAY_SUCCESS);
    else if (getExposedPulse(0)->getmAs()) setCompletedCode(exposure_completed_options::XRAY_PARTIAL_DOSE);
    else setCompletedCode(exposure_completed_options::XRAY_NO_DOSE);
    LogClass::logInFile("GENERATOR EXPOSURE RESULT:" + getExposureCompletedCode().ToString() + "-" + exposure_err_code.ToString());

    // Debug Prints
    if (exposure_err_code == exposure_completed_errors::XRAY_NO_ERRORS)  LogClass::logInFile("GENERATOR EXPOSURE: Successfully completed \n");
    else  if (getExposedPulse(0)->getmAs() == 0)    LogClass::logInFile("GENERATOR EXPOSURE ERROR (NO DOSE): Error code = " + exposure_err_code.ToString() + "\n");
    ...
    if (getCompressorMode() == compression_mode_option::CMP_RELEASE) PCB302::setCompressorUnlock();
             
    // Notify the AWS about the XRAY completed event
    awsProtocol::EVENT_XraySequenceCompleted();
           
    // Disable the Xray Button
    enableXrayPushButtonEvent(false);
    ...
    setXrayCompletedFlag(); // The Exposure is completed and the data are available
...
ExposureModule::exposure_completed_errors ExposureModule::aec_2d_exposure_procedure_demo(void) {
    exposure_pulse^ exposure_pulse;
    double kV;
    double mAs;
...
ExposureModule::exposure_completed_errors ExposureModule::man_ae_exposure_procedure_demo(void) { return ExposureModule::exposure_completed_errors::XRAY_COMMUNICATION_ERROR; }
ExposureModule::exposure_completed_errors ExposureModule::aec_ae_exposure_procedure_demo(void) { return ExposureModule::exposure_completed_errors::XRAY_COMMUNICATION_ERROR; }

ExposureModule::exposure_completed_errors ExposureModule::man_3d_exposure_procedure_demo(void) {
...
ExposureModule::exposure_completed_errors ExposureModule::aec_3d_exposure_procedure_demo(void) { return ExposureModule::exposure_completed_errors::XRAY_COMMUNICATION_ERROR; }
ExposureModule::exposure_completed_errors ExposureModule::man_combo_exposure_procedure_demo(void) { return ExposureModule::exposure_completed_errors::XRAY_COMMUNICATION_ERROR; }
ExposureModule::exposure_completed_errors ExposureModule::aec_combo_exposure_procedure_demo(void) { return ExposureModule::exposure_completed_errors::XRAY_COMMUNICATION_ERROR; }