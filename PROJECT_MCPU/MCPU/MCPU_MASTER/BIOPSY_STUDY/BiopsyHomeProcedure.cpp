...
#define PROCEDURE_X_MOVE_TO_TARGET_IMG ""
#define PROCEDURE_Y_MOVE_TO_TARGET_IMG ""
#define PROCEDURE_Z_MOVE_TO_TARGET_IMG ""
#define PROCEDURE_X_MOVE_TO_TARGET_INFO "X MOVING ..."
#define PROCEDURE_Y_MOVE_TO_TARGET_INFO "Y MOVING ..."
#define PROCEDURE_Z_MOVE_TO_TARGET_INFO "Z MOVING ..."


#define PROCEDURE_XSCROLL_CENTER_REQUEST_IMG ""
#define PROCEDURE_XSCROLL_CENTER_REQUEST_INFO "X-SCROLL TO CENTER"

#define PROCEDURE_XSCROLL_LEFT_REQUEST_IMG ""
#define PROCEDURE_XSCROLL_LEFT_REQUEST_INFO "X-SCROLL TO LEFT"

#define PROCEDURE_XSCROLL_RIGHT_REQUEST_IMG ""
#define PROCEDURE_XSCROLL_RIGHT_REQUEST_INFO "X-SCROLL TO RIGHT"

#define PROCEDURE_XSCROLLED_CENTER_IMG ""
#define PROCEDURE_XSCROLLED_LEFT_IMG ""
#define PROCEDURE_XSCROLLED_RIGHT_IMG ""

#define PROCEDURE_Y_FLIP_DOWN_REQUEST_IMG ""
#define PROCEDURE_Y_FLIP_DOWN_REQUEST_INFO "FLIP DOWN Y"
#define PROCEDURE_FLIP_Y_DOWN_OK_IMG ""
...
// --------------------------------------------------------------------------------------
	...
	case SET_VALID_SCROLL_WAIT_POSITIONING:
		...
		BIOPSY->showProcedureImage(PROCEDURE_HOME_VALID_XSCROLL_IMG, CONFIRM_INFO, INFO_ICON, true);
	...
	case YUP_EXECUTE_FLIP_UP:
		BIOPSY->showProcedureImage(PROCEDURE_FLIP_UP_EXECUTE_UP_IMG, PROCEDURE_FLIP_UP_EXECUTE_UP_INFO, ACTION_ICON, false);
		step = YUP_EXECUTE_FLIP_UP_WAIT_SENSOR;
		break;

	case YUP_EXECUTE_FLIP_UP_WAIT_SENSOR:
		if (!PCB325::isYUp()) break;
		BIOPSY->showProcedureImage(PROCEDURE_FLIP_UP_EXECUTED_IMG, CONFIRM_INFO, INFO_ICON, true);
		BIOPSY->getProcedureConfirmation(true); // reset the confirmation flag
	...
	case EXECUTE_XYZ_INIT:
		BIOPSY->showProcedureImage(PROCEDURE_X_MOVE_TO_TARGET_IMG, PROCEDURE_X_MOVE_TO_TARGET_INFO, INFO_ICON, false);
		if (!PCB325::moveX(X)) {
			step = PROCEDURE_ERROR;
			result = 4;
			break;
	...
	case XYZ_WAIT_X:
		...
		BIOPSY->showProcedureImage(PROCEDURE_Y_MOVE_TO_TARGET_IMG, PROCEDURE_Y_MOVE_TO_TARGET_INFO,INFO_ICON, false);
		if (!PCB325::moveY(Y)) {
			step = PROCEDURE_ERROR;
			result = 6;
			break;
	...
	case XYZ_WAIT_Y:
		...
		BIOPSY->showProcedureImage(PROCEDURE_Z_MOVE_TO_TARGET_IMG, PROCEDURE_Z_MOVE_TO_TARGET_INFO, INFO_ICON, false);
		if (!PCB325::moveZ(Z)) {
			step = PROCEDURE_ERROR;
			result = 8;
			break;
	...
	case EXECUTE_XSCROLL_INIT:
		...
		// Request to scroll
		if (target == home_positions::HOME_CENTER) {
			BIOPSY->showProcedureImage(PROCEDURE_XSCROLL_CENTER_REQUEST_IMG, PROCEDURE_XSCROLL_CENTER_REQUEST_INFO,ACTION_ICON, false);
		}else if (target == home_positions::HOME_LEFT) {
			BIOPSY->showProcedureImage(PROCEDURE_XSCROLL_LEFT_REQUEST_IMG, PROCEDURE_XSCROLL_LEFT_REQUEST_INFO,ACTION_ICON, false);
		}else BIOPSY->showProcedureImage(PROCEDURE_XSCROLL_RIGHT_REQUEST_IMG, PROCEDURE_XSCROLL_RIGHT_REQUEST_INFO,ACTION_ICON, false);
	...
	case EXECUTE_XSCROLL_WAIT_SENSOR:
		...
		if (target == home_positions::HOME_CENTER) {
			...
			BIOPSY->showProcedureImage(PROCEDURE_XSCROLLED_CENTER_IMG, CONFIRM_INFO,INFO_ICON, true);
		...
		if (target == home_positions::HOME_LEFT) {
			...
			BIOPSY->showProcedureImage(PROCEDURE_XSCROLLED_LEFT_IMG,  CONFIRM_INFO,INFO_ICON, true);
			step = EXECUTE_XSCROLL_WAIT_CONFIRMATION;
			BIOPSY->getProcedureConfirmation(true);
			break;
		}
		...
		if (target == home_positions::HOME_RIGHT) {
			...
			BIOPSY->showProcedureImage(PROCEDURE_XSCROLLED_RIGHT_IMG, CONFIRM_INFO,INFO_ICON, true);
			step = EXECUTE_XSCROLL_WAIT_CONFIRMATION;
			BIOPSY->getProcedureConfirmation(true);
			break;
		}
	...
	case EXECUTE_Y_FLIP_DOWN_INIT:
		...
		BIOPSY->showProcedureImage(PROCEDURE_Y_FLIP_DOWN_REQUEST_IMG, PROCEDURE_Y_FLIP_DOWN_REQUEST_INFO,ACTION_ICON, false);
		step = EXECUTE_Y_FLIP_DOWN_WAIT_SENSOR;
		break;
	
	case EXECUTE_Y_FLIP_DOWN_WAIT_SENSOR:
		if (PCB325::isYUp()) break;
		BIOPSY->showProcedureImage(PROCEDURE_FLIP_Y_DOWN_OK_IMG, CONFIRM_INFO,INFO_ICON, true);
