...
namespace R2CP
{
    ...
    void CaDataDicGen::Patient_SetupProcedureV6(unsigned char procId, unsigned char param){
        ...
        if (procId == ProcId_2D_NoDet_NoGrid) { 
        ...
        } else if (procId == ProcId_2D_NoDet_WithGrid) { 

            pData[0] = procId; // Proc Id
            pData[1] = ProcType_StandardRADMamo;   
            pData[2] = 0;   // Positioner Proc Type = not defined
            pData[3] = 1;   // Handswitch ID = not defined
            pData[4] = 0;   // Activation Mode: 1 = Push, 0 = Software
            pData[5] = DIRECT_WORKSTATION_ID;   
        ...
        } else if (procId == ProcId_2D_WithDet_NoGrid) { 
            pData[0] = procId; // Proc Id
            pData[1] = ProcType_StandardRAD;   
            pData[2] = 0;   // Positioner Proc Type = not defined
            pData[3] = 1;   // Handswitch ID = not defined
            pData[4] = 0;   // Activation Mode: 1 = Push, 0 = Software
            pData[5] = DYNAMIC_WORKSTATION_ID;  
            pData[6] = 1;   // Num Databank
            pData[7] = 1;   // Positioning Databank
            pData[8] = 1;   // Collimator Databank
            pData[9] = 1;   // Filter Databank
        ...
        }else if (procId == ProcId_2D_WithDet_WithGrid) {
            pData[0] = procId; // Proc Id
            pData[1] = ProcType_StandardRADMamo;
            pData[2] = 0;   // Positioner Proc Type = not defined
            pData[3] = 1;   // Handswitch ID = not defined
        ...
        }else if(procId == ProcId_2D_Aec_NoGrid){ 
            pData[0] = procId; // Proc Id
            pData[1] = ProcType_MultiEnergy;   
        ...
        else if (procId == ProcId_2D_Aec_WithGrid) {
            pData[0] = procId; // Proc Id
            pData[1] = ProcType_Aec_Mammography_2D;
            pData[2] = 0;   // Positioner = not defined
            pData[3] = 1;   // Handswitch ID = not defined
            pData[4] = 0;   // Activation Mode: 1 = Push, 0 = Software
            pData[5] = DYNAMIC_WORKSTATION_ID;
            ...
            pData[3] = 1;   // Handswitch ID = not defined
            pData[4] = 0;   // Activation Mode: 1 = Push, 0 = Software
            pData[5] = DYNAMIC_WORKSTATION_ID;   // Workstation
            pData[6] = 1;   // Num Databank
            pData[7] = 1;   // Positioning Databank
            pData[8] = 1;   // Collimator Databank
            pData[9] = 1;   // Filter Databank
    ...
    void CaDataDicGen::Patient_Activate2DProcedurePulse(bool detector, bool grid){
        ...
        if ((!detector) && (!grid)) pData[0] = ProcId_2D_NoDet_NoGrid;
        else if ((!detector) && (grid)) pData[0] = ProcId_2D_NoDet_WithGrid;
        else if ((detector) && (!grid)) pData[0] = ProcId_2D_WithDet_NoGrid;
        else if ((detector) && (grid)) pData[0] = ProcId_2D_WithDet_WithGrid;
    ...
    void CaDataDicGen::Patient_Activate2DAecProcedurePulse(bool grid){

        unsigned char pData[6];

        if(grid) pData[0] = ProcId_2D_Aec_WithGrid; // Proc Id
        else pData[0] = ProcId_2D_Aec_NoGrid; // Proc Id
    ...
    void CaDataDicGen::Patient_Activate2DAecProcedurePre(bool grid){
